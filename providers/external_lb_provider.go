...
type Provider interface {
	...
	HealthCheck() error
	// AddLBConfig adds a new load balancer configuration. It may
	// return the FQDN for the endpoint if supported by the provider.
	AddLBConfig(config model.LBConfig) (fqdn string, err error)
	// UpdateLBConfig updates the load balancer configuration. It may
	// return the FQDN for the endpoint if supported by the provider.
	UpdateLBConfig(config model.LBConfig) (fqdn string, err error)
	// RemoveLBConfig removes the specified load balancer configuration.
	RemoveLBConfig(config model.LBConfig) error
	// GetLBConfigs returns all load balancers configuration.
